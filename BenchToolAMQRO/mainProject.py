...
remote_check_multiple_ps1 = os.path.join(base_path, 'RemoteCheckMultipleBenchesV2.ps1')
...
def show_centered_message(title, message):
    ...
    y = root.winfo_y() + (root.winfo_height() // 2) - 50   # 50 is half of message box height (100/2)
...
def get_powershell_output(output_queue):
    ...
    command = ["powershell.exe", "-WindowStyle", "Hidden", "-ExecutionPolicy", "Bypass", "-File", remote_check_multiple_ps1]
...
def update_gui():
        ...
        while True:
            ...
            if connect_initiated:
                ...
                if (selected_computer_status == "Available" or selected_computer_status == "Not Initialized") and update_counter >= 1:
                    ...
                    command = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-Command", "$env:USERNAME"]
                ...
                # # After 2 updates, check the status and allow connection if available
                # if update_counter >= 1:
                #     close_all_popups()
                #     if selected_computer_status == "Available" or selected_computer_status == "Not Initialized":
                #         # Run the PowerShell command
                #         command = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-Command", "$env:USERNAME"]
                #         result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                ...
                #         remote_path = rf"\\{selected_computer}\BenchTool\LastUser.txt"
                #         lockFilePath = rf"\\{selected_computer}\BenchTool\RDP_Lock.txt"
                #         # Open the file in write mode at the remote path
                #         with open(lockFilePath, 'w') as file:
                #             file.write(f"Locked\n")
                #         connect()  # Proceed with connection
                #         time.sleep(3) # To keep the file 3 seconds
                ...
                #             print(f"{lockFilePath} has been removed successfully.")
                #         else:
                #             print(f"The file {lockFilePath} does not exist.")
                #         # Open the file in write mode at the remote path
                #         with open(remote_path, 'w') as file:
                #             file.write(f"{username}\n")

                #     # elif selected_computer_status == "Not Initialized":
                #     #     show_centered_message("Bench on hold", "Needs To Start Manually Remote Session")  # Show custom pop-up
                #     else:
                #         # messagebox.showinfo("Bench in Use", "Someone else is using the Bench")  # Show pop-up
                #         show_centered_message("Bench in Use", "Someone else is using the Bench")  # Show custom pop-up
                #     reset_connect_state()  # Reset the connection state
    ...
    root.after(100, update_gui)  # Check every 100 milliseconds
        ...
        if len(parts) == 6:  # Expecting six parts: computer, name, ip, status, uid, pswd
            ...
            if computer in labels:
                if labels[computer] is None:  # If label hasn't been created yet
                    ...
                    radio_button = tk.Radiobutton(frame, text=f"Select {name}", variable=selected_ip, value=ip)
